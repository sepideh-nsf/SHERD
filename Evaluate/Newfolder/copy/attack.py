...
def New_sort_sumtest(alpha, M, limit, bar, g, test_idx):
    ...
    :param g: the graph, used to calculate the.out_degrees of an node
...
def getScore(K, data):
    prob = normalize(th.FloatTensor(data._g.adjacency_matrix(transpose=False).to_dense()), p=1, dim=1)
    Random = prob
    for i in range(K - 1):
...
def getScoreGreedy(K, data, bar, num, beta):
    ...
    for i in range(K - 1):
        Random = th.sparse.mm(Random, prob)
    W = th.zeros(data[0].ndata['label'].shape[0], data[0].ndata['label'].shape[0])
    for i in range(data[0].ndata['label'].shape[0]):
    ...
    SCORE = W.sum(dim=0)
    ind = []
    l = [i for i in range(data[0].ndata['label'].shape[0]) if data._g.out_degrees(i) <= bar]
    for _ in range(num):
        cand = [(SCORE[i], i) for i in l]
        best = max(cand)[1]
        for neighbor in data._g.out_edges(best)[1]:
            if neighbor in l:
                l.remove(neighbor)
        ind.append(best)
        for i in l:
