...
from placenta_organs import Placenta as organ
...
class DefaultGraphConstructor(GraphConstructor):
    ...
    def construct(self, data_file, gt_file) -> Data:
        ...
        # Create graph
        # edge_index,_=grid(10,10)
        # edge_index,_=grid(int(np.max(coords[:,0])/1000),int(np.max(coords[:,1])/1000))
        # edges=np.array([coords[np.where((coords[:, 0] == item[0] + 1))] for item in coords])
        data = Data(
    ...
    def _build_edges(self, data):
        # knn graph
        # knn = KNNGraph(k=5)#, loop=False, force_undirected=True)(data)
        # data.batch=5
        ...
        delaunay_edge_index = triang.edges.astype("int32")
        # intersection graph
        # _, ncols1 = knn_edge_index.shape;
        # _, ncols2 = delaunay_edge_index.shape;
        # dtype1 = ", ".join([str(knn_edge_index.dtype)] * ncols1);
        # dtype2 = ", ".join([str(delaunay_edge_index.dtype)] * ncols2);
        #
        # intersection = np.intersect1d(
        #     knn_edge_index.view(dtype1), delaunay_edge_index.view(dtype2)
        # );
