...
class PGD(ModificationAttack):
    ...
    def modification(self,
                     adj,
                     index_target):
        adj_temp = sp.csr_matrix(adj)
        adj_attack = adj_temp.copy()
        # adj_attack = deepcopy(adj)
    ...
    def update_features(self,
        ...
        adj_attacked_tensor = utils.adj_preprocess(adj=adj_attack,
                                                   ...
                                                   model_type=model.model_type,
                                                   device=self.device)
        index_mod = index_target#np.random.choice(index_target, self.n_node_mod)
        model.eval()
        epoch_bar = tqdm(range(n_epoch), disable=not self.verbose)
        for i in epoch_bar:
            features_attack.requires_grad_(True)
            features_attack.retain_grad()
            pred = model(features_attack, adj_attacked_tensor)[0]
