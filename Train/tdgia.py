...
class TDGIA(InjectionAttack):
    ...
    def attack(self,
        ...
        adj_tensor = utils.adj_preprocess(adj=adj,
                                          ...
                                          device=self.device)
        pred_origin = model(features, adj_tensor)[0]
        ...
        while n_inject < self.n_inject_max:
            current_labels = F.softmax(model(features_attack, adj_tensor)[0], dim=1)
            # current_labels = model(features_attack, adj_tensor)[0]  ########################################
    ...
    def injection(self,
        ...
        if 'tdgia' in mode:
            ...
            deg = np.array(adj.sum(axis=0)) + 1.0
    ...
    def update_features(self,
        ...
        for i in epoch_bar:
            ...
            pred = model(features_concat, adj_attacked_tensor)[0]
